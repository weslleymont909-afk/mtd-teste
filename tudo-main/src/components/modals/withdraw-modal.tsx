...
export function WithdrawModal({
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numericValue > balance) {
      ...
      // Do not update state if value is invalid, just keep the raw value to show user input
      setAmount(rawValue)
    } else {
      setAmountError(null);
      setAmount(rawValue);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md rounded-2xl p-6 shadow-lg data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-bottom-3 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-bottom-3 duration-300 ease-out">
        ...
        <div className="my-8 space-y-6">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {pixKeyTypes.map(({ id, label, icon: Icon }) => (
                <button
                  ...
                  className={cn(
                    'flex flex-col items-center justify-center gap-2 rounded-lg border p-3 text-sm font-medium transition-all duration-200 ease-out',
                    selectedKeyType === id
                      ? 'border-primary bg-primary/5 text-primary ring-1 ring-primary scale-105 shadow-md'
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className="h-12 rounded-lg focus-visible:ring-primary/50"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="R$ 0,00"
              className={cn(
                'h-12 rounded-lg focus-visible:ring-primary/50',
        ...
        <Button
          ...
          className="h-12 w-full rounded-full text-base font-semibold transition-opacity duration-200 ease-out disabled:cursor-not-allowed disabled:opacity-50"
        >
          REALIZAR SAQUE
        </Button>
      </DialogContent>
